...
const App = () => {
		...
		try {
			...
			if (ethereum) {
				...
				const mintRecords = await Promise.all(names.map(async (name) => {
					const mintRecord = await contract.domainRec();
					const owner = await contract.domains(name);
					return {
						id: names.indexOf(name),
						name: name,
						record: mintRecord.name,
						email: mintRecord.email,
						owner: owner,
					};
				}));
		...
		try {
			...
			if (ethereum) {
				...
				if (receipt.status === 1) {
					...
					tx = contract.setRecord(domain, record, email);
					await tx.wait();
	...
	const updateDomain = async () => {
		...
		try {
			...
			if (ethereum) {
				...
				let tx = await contract.setRecord(domain, record, email);
				await tx.wait();
				console.log("Record set https://mumbai.polygonscan.com/tx/" + tx.hash);
	...
	const renderNotConnectedContainer = () => (
		<div className="connect-wallet-container">
			<img src="https://media.giphy.com/media/6vzwF4HgszGBLiOvdq/giphy.gif" alt="Ninja gif" />
		...
		<div className="App">
			<div className="container">
				...
				<div className="header-container">
					<header>
						<div className="left">
							<p className="title">🐱‍👤 Karen Complaining Service</p>
